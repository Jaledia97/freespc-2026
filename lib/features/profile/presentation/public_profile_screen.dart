...
final profileFriendsCountProvider = StreamProvider.family<int, String>((ref, uid) {
...
final profilePostsCountProvider = FutureProvider.family<int, String>((ref, uid) async {
  final db = FirebaseFirestore.instance;
  try {
    final wins = await db.collection('win_posts').where('userId', isEqualTo: uid).count().get();
    final checkins = await db.collection('check_ins').where('userId', isEqualTo: uid).count().get();
...
final profileGalleryProvider = FutureProvider.family<List<String>, String>((ref, uid) async {
  final db = FirebaseFirestore.instance;
  try {
    final QuerySnapshot winsSnapshot = await db.collection('win_posts').where('userId', isEqualTo: uid).orderBy('createdAt', descending: true).get();
    List<String> images = [];
...
class _PublicProfileScreenState extends ConsumerState<PublicProfileScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.profile.username, style: const TextStyle(fontWeight: FontWeight.bold)),
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          Hero(
                            ...
                            child: CircleAvatar(
                              ...
                              backgroundImage: widget.profile.photoUrl != null ? NetworkImage(widget.profile.photoUrl!) : null,
                              child: widget.profile.photoUrl == null ? const Icon(Icons.person, size: 45, color: Colors.white54) : null,
                            ),
                          ),
                          Expanded(
                            child: Row(
                              ...
                              children: [
                                Consumer(builder: (context, ref, _) {
                                  final postsCountAsync = ref.watch(profilePostsCountProvider(widget.profile.uid));
                                  final count = postsCountAsync.valueOrNull ?? 0;
                                  return _buildStatCol("Posts", count.toString());
                                }),
                                Consumer(builder: (context, ref, _) {
                                  final friendsCountAsync = ref.watch(profileFriendsCountProvider(widget.profile.uid));
                                  final count = friendsCountAsync.valueOrNull ?? 0;
                                  return _buildStatCol("Friends", count.toString());
                                }),
                                _buildStatCol("Points", widget.profile.points.toString()),
                              ],
                            ),
                          ),
                        ],
                      ...
                      if (widget.profile.bio != null && widget.profile.bio!.isNotEmpty) ...[
                        ...
                        Text(widget.profile.bio!, style: const TextStyle(color: Colors.white, fontSize: 14)),
                      ],
                      ...
                      if (widget.profile.squadIds.isNotEmpty)
                        Row(
                          children: [
                            ...
                            Text("Captain of ${widget.profile.squadIds.length} Squads", style: const TextStyle(color: Colors.white54, fontSize: 13, fontWeight: FontWeight.w600)),
                          ],
                        ),
                      ...
                      // Actions
                      if (!isSelf)
                        _FriendshipActionButton(currentUserId: currentUser.uid, targetUser: widget.profile)
                      else
                         SizedBox(
              ...
              if (_tabController.index == 0) ...[
                ...
                Consumer(
                  builder: (context, ref, _) {
                    final galleryAsync = ref.watch(profileGalleryProvider(widget.profile.uid));
        ...
        if (friendRecord == null) {
          ...
          return SizedBox(
            ...
            child: OutlinedButton(
              ...
              child: const Text("Requested", style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold)),
            ),
          );
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    final chat = await ref.read(messagingRepositoryProvider).createChat(
                      ...
                      {userAsync.uid: userAsync.username, targetUser.uid: targetUser.username}
                    );
                    if (context.mounted) {
