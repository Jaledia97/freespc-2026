      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Column(
          ...
          children: [
            ...
            SizedBox(height: 12),
            Text(
              "1. You hold all necessary local gaming licenses.",
              style: TextStyle(color: Colors.white70),
            ),
            SizedBox(height: 8),
            Text(
              "2. You accept full liability for this event.",
              style: TextStyle(color: Colors.white70),
            ),
            SizedBox(height: 8),
            Text(
              "3. No payment was required for entry.",
              style: TextStyle(color: Colors.white70),
            ),
          ],
        ),
    ...
    return Scaffold(
      ...
      body: sessionAsync.when(
        data: (session) {
          ...
          switch (status) {
            case 'roll_call':
              return _buildRollCallScreen(session);
            case 'locked':
              return _buildLockedScreen(session);
            case 'active_draw':
              return _buildDrawScreen(session);
            case 'winner_drawn':
              return _buildWinnerScreen(session);
            default:
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            "Ready to start '${widget.raffle.name}'",
  ...
  Widget _buildRollCallScreen(Map<String, dynamic> session) {
    ...
    return Column(
      children: [
        ...
        const SizedBox(height: 16),
        Text(
          "${participants.length} Analyzed Presence(s)",
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          Text(
            "${participants.length} Players Locked In",
