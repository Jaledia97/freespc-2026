          ...
          return CustomScrollView(
            slivers: [
              ...
              SliverAppBar(
                ...
                pinned: true,
                elevation: 0,
                title: const Text("Friends", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 24)),
                actions: [
                  IconButton(
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Text("My Status:", style: TextStyle(color: Colors.white70)),
                      const SizedBox(width: 8),
                      Container(
                        ...
                        child: DropdownButton<String>(
                          ...
                          isDense: true,
                          items: const [
                            DropdownMenuItem(value: 'Online', child: Row(children: [Icon(Icons.circle, color: Colors.greenAccent, size: 10), SizedBox(width: 6), Text('Online')])),
                            DropdownMenuItem(value: 'Away', child: Row(children: [Icon(Icons.circle, color: Colors.amber, size: 10), SizedBox(width: 6), Text('Away')])),
                            DropdownMenuItem(value: 'Offline', child: Row(children: [Icon(Icons.circle, color: Colors.grey, size: 10), SizedBox(width: 6), Text('Offline')])),
                          ],
                          onChanged: (val) {
              ...
              friendsAsync.when(
                data: (friends) {
                  ...
                  if (filteredFriends.isEmpty) {
                    return SliverFillRemaining(
                      ...
                      child: Center(
                        child: Text(
                          _searchQuery.isNotEmpty ? "No friends found matching '$_searchQuery'." : "No friends yet. Tap + to add some!", 
                  ...
                  return SliverList(
                    delegate: SliverChildBuilderDelegate(
                      (context, index) {
                        ...
                        if (friend.onlineStatus == 'Online') statusColor = Colors.greenAccent;
                        ...
                        return ListTile(
                          ...
                          title: Text(friend.username, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                          subtitle: friend.currentCheckInHallId != null 
                              ? const Text("📍 At a Bingo Hall", style: TextStyle(color: Colors.blueAccent, fontSize: 13)) 
                              : Text("${friend.firstName} ${friend.lastName}", style: const TextStyle(color: Colors.white54, fontSize: 13)),
                          trailing: Row(
                          ...
                          onTap: () {
                            Navigator.push(context, MaterialPageRoute(builder: (_) => PublicProfileScreen(profile: friend)));
                          },
                        );
                      },
                ...
                error: (e, st) => SliverFillRemaining(child: Center(child: Text("Error loading friends: $e", style: const TextStyle(color: Colors.red)))),
              )
            ],
          );
        },
        loading: () => const Center(child: CircularProgressIndicator(color: Colors.blueAccent)),
        error: (e, st) => const Center(child: Text("Error loading auth", style: TextStyle(color: Colors.red))),
      )
    );
  }
}
