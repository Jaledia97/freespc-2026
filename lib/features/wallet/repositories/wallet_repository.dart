...
final myRafflesStreamProvider =
    StreamProvider.family<List<RaffleTicketModel>, String>((ref, userId) {
      return ref.watch(walletRepositoryProvider).getMyRaffles(userId);
    });

final myTournamentsStreamProvider =
    StreamProvider.family<List<TournamentParticipationModel>, String>((
      ref,
      userId,
    ) {
...
final myTransactionsStreamProvider =
...
class WalletRepository {
  ...
  Stream<List<RaffleTicketModel>> getMyRaffles(String userId) {
    return _firestore
        ...
        .map((snapshot) {
          return snapshot.docs
              .map((doc) => RaffleTicketModel.fromJson(doc.data()))
              .toList();
        });
  }
  ...
  Stream<List<TournamentParticipationModel>> getMyTournaments(String userId) {
    return _firestore
        ...
        .map((snapshot) {
          return snapshot.docs
              .map((doc) => TournamentParticipationModel.fromJson(doc.data()))
              .toList();
        });
  }
